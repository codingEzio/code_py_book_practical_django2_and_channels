...
class AddressSelectionView(LoginRequiredMixin, FormView):
    """
    About `main:checkout_done`
        A simple success page which is called 'order_done.html'

    About `get_from_kwargs`
        Extract the 'user' & then assign to the response
    """
...
class OrderView(UserPassesTestMixin, FilterView):
    ...
    def test_func(self):
        """
        Restrict access by using `UserPassesTestMixin` & `test_func`.

        That is
            Only the admin users could access the `order-dashboard` page.
        """
